...
- destruct m as [| m'].
  + reflexivity.
  + Search plus. Admitted. 
(**rewrite PeanoNat.Nat.add_succ_r in h. discriminate h.**)
...
intros n. induction n as [|n'].
  - (* n = O *)
  intros m H. Search ( _ * _ = _ ). rewrite mult_0_l in H. left. apply H.
  - (* n = S n' *) intros m. induction m as [| m].
    + intros H. simpl in H. Search ( _ * _ = _ ). rewrite mul_0_r in H. right. apply H.
...
Theorem or_commut : forall P Q : Prop,
  P \/ Q  -> Q \/ P.
Proof.
intros P Q. intros [HP|HQ].
- right. apply HP.
- left. apply HQ.
(* apply or_comm. apply PQ. *)
Qed.
...
Proof.
intros P Q H nQ.
  unfold not. unfold not in nQ. intros P'. apply nQ. apply H. apply P'.
Qed.
...
- (* -> *)intros [H1|H2].
  ...
  + split.  
    * right. apply H2.
    * right. apply H2.
- (* anfterdom's contribution <- *) intros H. inversion H.
    + destruct H1.
      * left. apply H1.
      * destruct H0.
        { left. apply H0. }
...
intros x P Q.
split.
- (*->*)intros H. destruct H. destruct H as [HP|HQ].
  + left. exists x0. apply HP.
  + right. exists x0. apply HQ.
- (*<-*) intros H. destruct H as [HP|HQ].
  + destruct HP. exists x0. left. apply H.
  + destruct HQ. exists x0. right. apply H.
Qed.
  ...
  - induction l as[| x' l' IHl'].
    ...
    + simpl. intros [ H | H].
      * exists x'. split.
        { apply H. }
        { left. reflexivity. }
      * apply IHl' in H. destruct H as [x1 H1]. exists x1. split.
        { destruct H1. apply H. }
        { right. destruct H1. apply H0. }
  -  induction l as[| x' l' IHl'].
    + simpl. intros [x h]. apply h.
    + intros h.  destruct h. destruct H.  simpl in H0. simpl. destruct H0.
...
Theorem in_app_iff1 : forall A l l' (a:A),
