...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
intros A l m n. induction l as [| h l'].
-simpl. reflexivity.
-simpl. rewrite -> IHl'. reflexivity.
Qed.
...
match l with 
| nil => ([],[])
| (x1,x2) :: y' => (x1 :: fst (split y'), x2 :: snd (split y'))
end.
...
Example test_even_gtr:
  even 8 && negb (8 <=? 7) = true.
Proof. reflexivity. Qed.
...
Fixpoint negb_filter {X:Type} (test: X->bool) (l:list X) : list X :=
  match l with
  | [] => []
  | h :: t =>
    if negb (test h) then h :: (negb_filter test t)
    else negb_filter test t
  end.
...
(filter test l, negb_filter test l).
