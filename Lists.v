...
Fixpoint oddmembers (l:natlist) : natlist :=
 match l with
  | nil => nil
  | O :: ls => oddmembers ls
  | h :: ls => if odd h then h :: oddmembers ls else oddmembers ls
end. 
...
Fixpoint countoddmembers (l:natlist) : nat :=
  match l with 
  | nil => 0
  | O :: ls => countoddmembers ls
  | h :: ls => if odd h then S(countoddmembers ls) else countoddmembers ls
 end.
...
Fixpoint count (v : nat) (s : bag) : nat :=
  ...
  | nil => 0
  | 0:: b => count v b
  | h :: b => if eqb h v then S(count v b) else count v b
end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s with 
  | nil => nil
  | h :: s1 => if eqb v h then s1 else h :: remove_one v s1
end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
 match s with 
  | nil => nil
  | h :: s1 => if eqb v h then remove_all v s1 else h :: remove_all v s1
end.
...
match s1 with
|nil => true
| h :: s =>  member h s2 && subset s (remove_one h s2)
end.
...
-simpl. rewrite -> app_assoc. rewrite -> IHl'. rewrite -> app_assoc. reflexivity.
Qed.
...
| _ , nil => false
| nil, _ => false
| n1 :: l1', n2 :: l2' => if eqb n1 n2 then eqblist l1' l2' else false
end. 
...
Theorem count_member_zero : forall (s : bag),
  count 0 s <=? (count 0 s) = true.
Proof.
intros s. induction s as [| n s IHs'].
- simpl. reflexivity.
- simpl. induction n.
...
-simpl. reflexivity.
-simpl. induction n.
  + simpl. rewrite -> count_member_zero. reflexivity.
  + simpl. rewrite IHs'. reflexivity.
Qed.
