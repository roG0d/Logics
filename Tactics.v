...
- intros m. destruct m as [| m'].
  + reflexivity.
  + intros eq1. simpl in eq1. inversion eq1.
- intros m eq1. destruct m as [| m'].
  + simpl in eq1. inversion eq1.
  + apply f_equal. apply IHn'. inversion eq1. rewrite <- plus_n_Sm in H0. rewrite <- plus_n_Sm in H0. inversion H0. reflexivity.
Qed.
...
- destruct ( f (f b)) eqn: h2.
  + rewrite h1 in h2. rewrite -> h2. apply h2.
  + rewrite h1 in h2. rewrite -> h2. rewrite <- h2. destruct (f ( f (f b))) eqn:h3.
    * rewrite h1 in h3. apply h3.
    * rewrite h1 in h3. rewrite h2 in h3. rewrite -> h2. rewrite -> h3. symmetry in h3. rewrite h3 in h2. Admitted.
    (* to be  continued*)
